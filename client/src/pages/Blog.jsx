    ...
    useEffect(() => {
        const getCat = async () => {
            try {
                ...
                setBlogs(resData.allBlogs.reverse()); // Reverse the order
                setLoading(false);
            } catch (error) {
                console.log(error);
                handleAlert('Failed to fetch Blogs');
            }
        };
        getCat();
    }, [handleAlert]);
                            ...
                            <Link to={`/${handleLink(blog.cat.catName).toLowerCase()}/${blog.slug}`} key={blog._id}>
                                <div className="bar-card">
                                    <div className="bc-img-ctrl">
                                        <img
                                            src={blog.image || "https://buffer.com/library/content/images/size/w1200/2023/10/free-images.jpg"}
                                            alt={blog.title}
                                        />
                                    </div>
